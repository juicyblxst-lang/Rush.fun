...
const select="id,market_id,thesis_id,author_id,body,created_at,profiles:author_id(id,username,display_name,avatar_url,bio,created_at)";
...
export async function createPost(userId:string,input:{marketId?:string;thesisId?:string;body:string}){
  ...
  const {data,error}=await adminDb.from("posts").insert({author_id:userId,market_id:marketId??null,thesis_id:input.thesisId??null,body:input.body.trim()}).select(select).single();
...
export async function listPosts(externalMarketId:string){
  ...
  const {data,error}=await adminDb.from("posts").select(select).eq("market_id",market.data.id).order("created_at",{ascending:false}).limit(100);
  if(error) throw new AppError("DB_ERROR",error.message,500);
  return data??[];
}
export async function addComment(userId:string,input:{postId?:string;thesisId?:string;body:string}){
  ...
  const {data,error}=await adminDb.from("comments").insert({author_id:userId,post_id:input.postId??null,thesis_id:input.thesisId??null,body:input.body.trim()}).select("id,post_id,thesis_id,author_id,body,created_at,profiles:author_id(id,username,display_name,avatar_url,bio,created_at)").single();
  if(error) throw new AppError("DB_ERROR",error.message,500);
  return data;
}