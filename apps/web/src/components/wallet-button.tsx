...
export function WalletButton(){
  ...
  if(isConnected)return <div><button disabled={signing||!supabaseConfigured} onClick={async()=>{
    if(!address)return;
    if(!supabaseConfigured){setMessage("Authentication is not configured.");return;}
    const {data}=await supabase.auth.getSession();
    if(!data.session){setMessage("Sign in to link this wallet.");return;}
    try{
      const challenge=await api.walletChallenge(data.session.access_token,address);
      if(new Date(challenge.expiresAt).getTime()<=Date.now()){setMessage("Wallet link challenge expired. Try again.");return;}
      const signature=await signMessageAsync({message:challenge.message});
      await api.linkWallet(data.session.access_token,address,challenge.challengeId,challenge.message,signature);
